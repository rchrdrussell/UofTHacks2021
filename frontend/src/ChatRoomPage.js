...
function ChatRoomPage() {
    ...
    const handleSubmit = async evt => {
        ...
        data.getChatRoomName = getChatData().getChatRoomName;
    ...
    const connectToRoom = () => {
        socket.on("connect", data => {
            socket.emit("join", getChatData().getChatRoomName);
        });
    ...
    const getMessages = async () => {
        const response = await getChatRoomMessages(getChatData().getChatRoomName);
    ...
    return (
        <div className="chat-room-page">
            <h1>
                Chat Room: {getChatData().getChatRoomName}. Chat Handle:{" "} {getChatData().handle}
            </h1>
